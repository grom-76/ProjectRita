/*
	Compression décompression utilise le moteur 7z 
	(methode LZMA )

	
site : https://www.7-zip.org/download.html

Recup le 10/06/2022 ver :  21.07 (2021-12-26):

TODO : Nouvelle version 2022-07-15 7-Zip 22.01

*/

namespace RitaEngine.Base.IO.Formats.LZMA
{
	using System;
	using System.IO;
    ...
    namespace SevenZip
	{
		...
		public enum CoderPropID
		{
			...
			BlockSize,
			/// <summary>
			/// Specifies number of postion state bits for LZMA (0 inferieur ou égal à x inferieur ou égal à 4).
			/// </summary>
			PosStateBits,
			/// <summary>
			/// Specifies number of literal context bits for LZMA (0 inferieur ou égal à x inferieur ou égal à 8).
			/// </summary>
			LitContextBits,
			/// <summary>
			/// Specifies number of literal position bits for LZMA (0 inferieur ou égal à x inferieur ou égal à 4).
			/// </summary>
			LitPosBits,
			/// <summary>
	...
	namespace SevenZip.Buffer
	{
		...
		public class InBuffer
		{
			...
			/// <summary> . </summary>
			public bool ReadByte( out byte? b) // check it
			{
				if (m_Pos >= m_Limit)
                {
					b = 0;
			...
			public byte ReadByte()
				=>  (m_Pos >= m_Limit && !ReadBlock() )? (byte)0xFF : m_Buffer[m_Pos++];
	...
	{
		#region Ranger coder.cs in RangeCoder Folder
		...
		class Encoder
		{
			...
			public void SetStream(System.IO.Stream stream) =>	Stream = stream;
			...
			public long GetProcessedSizeAdd()
				=> _cacheSize +	Stream!.Position - StartPosition + 4;
		...
		class Decoder
		{
			...
			{
				uint newBound = (Range >> numTotalBits) * size0;
				uint symbol = (Code < newBound) ? (uint)0 : (uint)1;
				Range = (Code < newBound) ?  newBound : Range - newBound;
				Code = (Code < newBound) ? Code : Code - newBound;
				Normalize();
				return symbol;
			}
		}
		...
		#region RangerCoderBit in RangeCoder Folder
		struct BitEncoder
		{
			...
			public void UpdateModel(uint symbol)=> 	Prob = symbol == 0 ? Prob + (kBitModelTotal - Prob) >> kNumMoveBits: Prob - ( Prob >> kNumMoveBits);
		...
		struct BitDecoder
		{
			...
			public void UpdateModel(int numMoveBits, uint symbol)
				=> Prob = symbol == 0 ? Prob + ((kBitModelTotal - Prob) >> numMoveBits ): Prob - ((Prob) >> numMoveBits) ;
		...
		#region RangerCoderBitTree in RangeCoder Folder
	...
	namespace SevenZip.Compression.LZMA
	{
		...
		public class Decoder : ICoder, ISetDecoderProperties // ,System.IO.Stream
		{
			...
			{
				if (m_DictionarySize == dictionarySize) return ;
		...
		public class Encoder : ICoder, ISetCoderProperties, IWriteCoderProperties
		{
			...
			UInt32 GetRepPrice(UInt32 repIndex, UInt32 len, Base.State state, UInt32 posState)
				=>  _repMatchLenEncoder.GetPrice(len - Base.kMatchMinLen, posState)  + GetPureRepPrice(repIndex, state, posState);
			...
			UInt32 GetOptimum(UInt32 position, out UInt32 backRes)
			{
				...
				numAvailableBytes = (numAvailableBytes > Base.kMatchMaxLen) ? Base.kMatchMaxLen:numAvailableBytes ;
			...
			readonly static string[] kMatchFinderIDs = 
			{
				"BT2",
				"BT4",
			};
	...
	namespace SevenZip.Compression.LZ
	{
		...
		public class BinTree : InWindow, IMatchFinder
		{
			...
			public void SetType(int numHashBytes)
			{
				...
				kNumHashDirectBytes = HASH_ARRAY ? (uint)0 : 2;
				kMinMatchCheck = HASH_ARRAY ? (uint)4 :2 + 1 ;
				kFixHashSize = HASH_ARRAY ? kHash2Size + kHash3Size:0 ;
			}
			/// <summary> . </summary>
			...
			static void NormalizeLinks(UInt32[] items, UInt32 numItems, UInt32 subValue)
			{
